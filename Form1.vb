...
Public Class Form1
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click, NumericUpDown5.ValueChanged, NumericUpDown6.ValueChanged, NumericUpDown7.ValueChanged
        Dim sb As New StringBuilder
        Dim C = NumericUpDown7.Value / 10 ^ 9       '[nF]->[F]
        Dim L = NumericUpDown5.Value / 10 ^ 6       '[uH]->[H]
        Dim R = NumericUpDown6.Value                '[ohm]    
        Dim f = 1 / Math.Sqrt(L * C)                '[Hz]

        Dim xl = 2 * Math.PI * f * L                '[ohm]
        Dim xc = 1 / (2 * Math.PI * f * C)          '[ohm]
